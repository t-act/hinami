...
// .wrangler/tmp/bundle-loKixq/strip-cf-connecting-ip-header.js
...
var SESSION_TTL = 60 * 60 * 24 * 365;
function resolveCategory(category, creatorRole, viewerRole) {
  if (category === "together")
    return "together";
  if (creatorRole === viewerRole)
    return category;
...
app.post("/couples", async (c) => {
  ...
  await db.prepare("INSERT INTO couples (id, token, created_at) VALUES (?, ?, ?)").bind(coupleId, inviteToken, now).run();
  await db.prepare(
    "INSERT INTO sessions (id, couple_id, role, created_at, expires_at) VALUES (?, ?, ?, ?, ?)"
  ).bind(sessionId, coupleId, "creator", now, expiresAt).run();
...
app.post("/couples/join/:token", async (c) => {
  ...
  await db.prepare(
    "INSERT INTO sessions (id, couple_id, role, created_at, expires_at) VALUES (?, ?, ?, ?, ?)"
  ).bind(sessionId, couple.id, "partner", now, expiresAt).run();
  setCookie(c, SESSION_COOKIE, sessionId, {
    httpOnly: true,
    path: "/",
    maxAge: SESSION_TTL,
...
app.get("/couples/me", requireSession, async (c) => {
  ...
  const couple = await c.env.DB.prepare("SELECT * FROM couples WHERE id = ?").bind(session.couple_id).first();
  if (!couple) {
  ...
  return c.json({
    couple_id: couple.id,
    role: session.role,
    invite_token: session.role === "creator" ? couple.token : void 0
  });
});
app.get("/events", requireSession, async (c) => {
  ...
  ).bind(session.couple_id, `${month}-%`).all();
...
app.post("/events", requireSession, async (c) => {
  const session = c.get("session");
  const body = await c.req.json();
  const { title: title2, category, date, start_time, end_time } = body;
  if (!title2 || !category || !date) {
  ...
  await c.env.DB.prepare(
    `INSERT INTO events
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).bind(id, session.couple_id, session.id, title2, category, date, start_time ?? null, end_time ?? null, now, now).run();
  return c.json(
    { id, title: title2, category, date, start_time: start_time ?? null, end_time: end_time ?? null },
    201
  );
});
app.patch("/events/:id", requireSession, async (c) => {
  ...
  ).bind(eventId, session.couple_id).first();
  if (!existing) {
...
app.delete("/events/:id", requireSession, async (c) => {
  ...
  ).bind(eventId, session.couple_id).run();
...
// .wrangler/tmp/bundle-loKixq/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-loKixq/middleware-loader.entry.ts
